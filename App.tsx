...
const App: React.FC = () => {
      ...
      try {
        ...
        if (projectId) {
          ...
          if (project) {
            ...
            // Auto-scroll to projects if it's a deep link
            setTimeout(() => {
  ...
  return (
    <div className="min-h-screen relative selection:bg-indigo-500/30 overflow-x-hidden w-full">
      ...
      {/* Hero Section */}
      <header className="relative pt-28 md:pt-44 pb-20 md:pb-32 px-4 md:px-8">
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
          <div className="reveal inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-[8px] md:text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-400 mb-6 shadow-sm">
          ...
          <div className="reveal flex flex-col items-center gap-2 mb-6 md:mb-8">
            <h1 className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-black font-outfit tracking-tighter leading-[1.1] text-white px-2">
              Architecting <br className="hidden sm:block"/><span className="text-gradient">Intelligent Systems</span>
            </h1>
            <h2 className="text-base md:text-2xl font-medium text-gray-400 tracking-tight font-outfit mt-2">
              Hi, I'm <span className="text-white font-black">Pankaj Kapri</span>
            </h2>
          </div>
          
          <p className="reveal max-w-xl text-xs sm:text-base md:text-lg text-gray-400 mb-8 md:mb-10 leading-relaxed px-6 opacity-80">
          ...
          <div className="reveal flex flex-col sm:flex-row gap-3 sm:gap-4 items-center w-full sm:w-auto px-10 sm:px-0">
            <Magnetic strength={0.2} className="w-full sm:w-auto">
              <button onClick={handlePortfolioClick} className="block text-center relative overflow-hidden px-8 py-3.5 rounded-full bg-indigo-600 text-white text-[9px] md:text-xs font-black uppercase tracking-[0.2em] transition-all hover:bg-white hover:text-gray-950 group w-full sm:w-auto">
                <span className="relative z-10">Portfolio</span>
              </button>
            </Magnetic>
            <Magnetic strength={0.2} className="w-full sm:w-auto">
              <a href={CV_LINK} target="_blank" className="block text-center relative overflow-hidden px-8 py-3.5 rounded-full bg-white/5 border border-white/10 text-white text-[9px] md:text-xs font-black uppercase tracking-[0.2em] transition-all hover:bg-white hover:text-gray-950 w-full sm:w-auto">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="space-y-16 md:space-y-40 relative">
            {EXPERIENCES.map((exp, idx) => {
              ...
              // Explicit theme mapping to prevent dynamic Tailwind class failure
              const themeStyles = {
                emerald: { 
                  bg: 'bg-emerald-500', 
              ...
              return (
                <div key={exp.id} onClick={() => setSelectedExperience(exp)} className="reveal relative flex flex-col md:flex-row items-center cursor-pointer group">
                  ...
                  <div className={`w-full md:w-[42%] ${isEven ? 'md:mr-auto' : 'md:ml-auto'}`}>
                    <div className={`glass p-8 md:p-10 rounded-[2.5rem] border-white/5 transition-all duration-500 hover:bg-white/[0.05] hover:border-white/20 group-hover:-translate-y-2 shadow-2xl relative overflow-hidden`}>
                        ...
                        <p className="text-gray-400 text-sm md:text-base leading-relaxed opacity-70 group-hover:opacity-100 transition-opacity mb-8">
                          {exp.description}
                        </p>
                      ...
                      {[
                        ...
                        <Magnetic strength={0.3} key={s.name}>
                          <a 
                            ...
                            className={`w-9 h-9 md:w-10 md:h-10 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center transition-all duration-300 group hover:-translate-y-1.5 hover:shadow-2xl hover:text-white ${s.hoverClass || 'hover:bg-indigo-600'}`}
                            aria-label={s.name}
                          >
                  ...
                  <form onSubmit={handleFormSubmit} className={`space-y-4 transition-opacity duration-300 w-full ${formStatus === 'submitting' ? 'opacity-40 pointer-events-none' : ''}`}>
                    ...
                    <div className="pt-2">
                       <button 
                         ...
                         className={`w-full py-4 rounded-xl font-black text-[8px] md:text-[9px] uppercase tracking-[0.3em] transition-all duration-500 flex items-center justify-center gap-3 relative overflow-hidden ${
